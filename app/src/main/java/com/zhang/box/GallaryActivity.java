...
public class GallaryActivity extends Activity {
    private static final String LogTag = "GallaryActivity";
...
//				SetVideo(mPosition);
    ...
    private void setGridView() {
        ...
        Log.e("" + LogTag, "" + UserInfo.proAlllist.size());
        gridView.setAdapter(adapter);
        adapter.notifyDataSetChanged();
    }

    //初始化主页九个展示货品
    private List<UserInfo> initList() {
        list = new ArrayList<UserInfo>();
        List<UserInfo> exList = new ArrayList<UserInfo>();
        UserInfo user ;
        String huodaoInfo,resultInfo;
        int size = list.size();
        if (UserInfo.proAlllist != null && UserInfo.proAlllist.size() > 0) {
            for (int i = 0; i < UserInfo.proAlllist.size(); i++){
                user = UserInfo.proAlllist.get(i);
                int huogui = Integer.parseInt(user.prohuogui);
                int hdids = user.hdid;
                //
                huodaoInfo = MainHandler.getGoodsInfo(huogui, hdids);
...
//		}if(andex%2 != 0){
//			videoUri = Uri.parse("android.resource://" + getPackageName() + "/"+ R.raw.huagong);
//		}
